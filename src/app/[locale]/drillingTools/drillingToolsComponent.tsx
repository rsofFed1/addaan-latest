...
import { DrillingToolsProductsData } from "@/constant/SuppliersProductsData/DrillingTools"
...
export const DrillingToolsProductsComponent = ({ showHeader = true } : { showHeader?: boolean }) => {
    const [expandedCategory, setExpandedCategory] = useState<string | null>("drilling-tools")
                        ...
                        <div className="max-w-[1400px] mx-auto px-4 z-10 relative">
                            <div className="text-center">
                                <div className="inline-flex items-center justify-center mb-6">
                                    ...
                                    <span className="text-white font-semibold uppercase tracking-wider text-sm"> {t('Products.drillingTools.title')} </span>
                                    <div className="w-12 h-1 bg-[#13A7DC] m-4" />
                                </div>
                                <h2 className="text-4xl md:text-5xl font-bold text-white mb-6"> {t('Products.drillingTools.heading')} </h2>
                                <p className="text-lg text-gray-200 max-w-2xl mx-auto leading-relaxed"> {t('Products.drillingTools.description')} </p>
                            </div>
                        </div>
                    </section>
                )}
                <section id="products" className="py-20 relative overflow-hidden" style={{ backgroundImage: "url('/images/artwork/artwork-7.webp')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: 'rgba(0, 0, 0, 0.8)', backgroundBlendMode: 'multiply' }}  >
                    <div className="max-w-[1400px] mx-auto px-4 relative z-10">
                        <div className="space-y-6 max-w-[1400px] mx-auto">
                            {DrillingToolsProductsData.map((category) => (
                                <div
                                    key={category.id}
                                                                ...
                                                                <div className="flex flex-col justify-between h-full">
                                                                    <div>
                                                                        <div className="w-full h-[400px] mb-4 rounded-lg flex items-center justify-center shadow ">
                                                                            {product.image ? (<Image height={500} width={500} src={product.image} alt={product.name} className="object-cover h-full w-full" />) : (<div className="text-gray-400">No Image Available</div>)}
                                                                        </div>
                                                                    ...
                                                                    <div className="mt-3 pt-4 border-t border-gray-300">
                                                                        <EnquiryModel product={product} />
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ))}
