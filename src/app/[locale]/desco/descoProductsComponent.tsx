...
import { DescoProductsData } from '@/constant/SuppliersProductsData/DescoProducts'
...
export default function DescoProducts() {
    return (
        <DescoProductsComponent showHeader={true} />
    )
}

export const DescoProductsComponent = ({ showHeader = true } : { showHeader: boolean })=> {
    const [expandedCategory, setExpandedCategory] = useState<string | null>("desco")
    const t = useTranslations()

    return (
        <div className={cn("bg-black", showHeader ? "pt-[80px] md:pt-[112px]" : "pt-0")}>
            {showHeader && (
                    ...
                    <div className="max-w-[1400px] mx-auto px-4 z-10 relative">
                        <div className="text-center">
                            <div className="inline-flex items-center justify-center mb-6">
                                ...
                                <span className="text-white font-semibold uppercase tracking-wider text-sm"> {t('Products.desco.title')} </span>
                                <div className="w-12 h-1 bg-[#13A7DC] m-4" />
                            </div>
                            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6"> {t('Products.desco.heading')} </h2>
                            <p className="text-lg text-gray-200 max-w-2xl mx-auto leading-relaxed"> {t('Products.desco.description')} </p>
                        </div>
                    </div>
                </section>
            )}
            <section id="products" className="py-20 relative overflow-hidden" style={{ backgroundImage: "url('/images/artwork/artwork-7.webp')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: 'rgba(0, 0, 0, 0.8)', backgroundBlendMode: 'multiply' }}  >
                <div className="max-w-[1400px] mx-auto px-4 relative z-10">
                    <div className="space-y-6 max-w-[1400px] mx-auto">
                        {DescoProductsData.map((category) => (
                            <div
                                key={category.id}
