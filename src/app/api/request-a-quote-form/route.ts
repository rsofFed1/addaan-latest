...
import { RequestQuoteFormEmailTemplate } from "./RequestQuoteEmailTemplate";
...
export async function POST(request: NextRequest) {
  try {
    const body: RequestQuoteFormData = await request.json();
    const { name, description, email } = body;

    RequestQuoteFormSchema.parse(body);
